...
type Config struct {
	...
	EnableServerStatus string `yaml:"enable_server_status" default:"yes"`
	// Credentials for Status Access
	ServerStatusUsername string `yaml:"server_status_username" default:"admin"`
	ServerStatusPassword string `yaml:"server_status_password" default:"admin_password"`

	//
	EnableSSL string `yaml:"enable_ssl" default:"yes"`
	...
	SSLCertFilePath string `yaml:"ssl_cert_file_path"`
	// This is the path where the ssl key file is being read, if no path provided, then an autogenerated certificate
	//will be made and used
	SSLKeyFilePath string `yaml:"ssl_key_file_path"`
	// Gives permission to autogenerate certificates if are missing
	SSLAutoGenerateCerts string `yaml:"ssl_auto_generate_certs" default:"yes"`

	// Allow Listening on HTTP only without encryption
	EnableUnsecure string `yaml:"enable_unsecure" default:"yes"`
	// Websocket param -> Enables compression between peers (client->server, server->client)
	EnableCompression string `yaml:"enable_compression" default:"yes"`
	// Websocket param
	ReadBufferSize uint64 `yaml:"read_buffer_size" default:"1024"`
	// Websocket param
	WriteBufferSize uint64 `yaml:"write_buffer_size" default:"1024"`
	// HTTP Listening
	ListeningAddresses []string `yaml:"listening_addresses"`
	// HTTPS Listening
	ListeningAddressesSSL []string `yaml:"listening_addresses_ssl"`
