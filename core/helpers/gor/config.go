...
type ReRunOnRecoverOptions struct {
	...
	RunAfterDuration time.Duration `yaml:"run_after_duration" default:"100000000"`
	// How many panics it should be allowed to happen... -1 is infinite!
	MaxNrOfPanics int `yaml:"max_nr_of_panics" default:"-1"`
}
...
type Config struct {
	...
	// It's a boolean
	AutoRecover interface{} `yaml:"auto_recover" default:"yes"`
	...
	// It's a boolean
	ReRunOnRecover interface{} `yaml:"rerun_on_recover" default:"no"`
	// This is the recovery options related to ReRunOnRecover
	ReRunOnRecoverOptions ReRunOnRecoverOptions `yaml:"rerun_on_recover_options"`

	// Add how many times it should run! default it's 1, -1 it's infinite, any number of times!
	RunTimes int `yaml:"run_times" default:"1"`
	// AutoRun -> It's a Boolean, Should it run on creation!?
	AutoRun interface{} `yaml:"auto_run" default:"yes"`
