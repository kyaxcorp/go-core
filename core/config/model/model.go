...
type Model struct {
	...
	Application struct {
		...
		CurrentWorkingDirectory string `yaml:"current_working_directory" default:""`
		...
		IfEmptyCWDSetToExecPath string `yaml:"if_empty_cwd_set_to_exec_path"  default:"yes"`
		// Stage -> prod, dev, beta, testing etc...
		Stage string `yaml:"stage"  default:"dev"`
		// App Data Path (all other subfolders will be stored there...)
		AppDataPath string `yaml:"app_data_path"  default:".appdata"`
		// Where the PIDs are saved!
		PIDsPath string `yaml:"pids_path"  default:"pids"`
		// Where the FileLocks are saved
		LocksPath string `yaml:"locks_path"  default:"locks"`
		// Temporary folder...
		TempPath string `yaml:"temp_path" default:"tmp"`
		// Storage Path -> where the data will be located
		StoragePath string `yaml:"storage_path"  default:"storage"`
		...
		CertsPath string `yaml:"certs_path"  default:"certs"`
		...
		ConfigsBackupPath string `yaml:"configs_backup_path"  default:"configs_backup"`
		// How many seconds should wait until the app gracefully shuts down all the services before os.exit(0)
		OnShutdownWaitSeconds int `yaml:"on_shutdown_wait_seconds"  default:"2"`
		// This is the default timezone... wherever it runs, when the app starts, it will override the timezone with this value
		TimeZone string `yaml:"time_zone"  default:"UTC"`
		// This param disables the override function on bootstrap only if necessary!
		DisableTimezoneOverride string `yaml:"disable_timezone_override"  default:"no"`
	}
	// Different connections to different services
	Clients struct {
		DefaultDBClient string `yaml:"default_db_client"  default:"cockroach"`
		MySQL           struct {
			DefaultConn struct {
				// This is the default Instance Name
				InstanceId string `yaml:"instance_id"  default:"default"`
			}
		...
		RPC struct {
			// Channel Name
			DefaultInstanceName string `yaml:"default_instance_name"  default:"default"`
			Instances           struct {
			}
		}
		WebSocket struct {
	...
	Logging struct {
		...
		AppLogLevel int `yaml:"level" default:"4"`

		LogsPath string `yaml:"logs_path"default:"logs"`
		// This is the default channel
		DefaultChannel string `yaml:"default_channel"  default:"default"`
